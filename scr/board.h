...
// Incremental state shared by the handcrafted evaluator and future NNUE.
...
// NNUE does not have a network loaded yet. This keeps the lifecycle hook in
// place without committing the board representation to a hidden-layer size.
struct NNUEAccumulatorState {
    uint64_t feature_version = 0;
...
struct Board {
    ...
    int  white_king_sq;
    int  black_king_sq;
    IncrementalEvalState incremental_eval;
    NNUEAccumulatorState nnue_state;
    ...
    void nnue_feature_added(int sq, int piece);
